...
public class NavigationMenu extends HtmlContainer {
    ...
    String _groupname;
    HtmlTable _table;
    HtmlStyle _linkstyle,_linkhoverstyle,_linkcellhoverstyle,_cellStyle,_cellHoverStyle,_selectedCellStyle,_selectedStyle,_selectedHoverStyle,_selectedCellHoverStyle,_fontStyle;
    HtmlStyle _linkfontstyle,_linkfonthoverstyle,_selectedFontStyle,_selectedFontHoverStyle;
    ...
    Vector _groups;
    int _NavTableBorder,_NavTableCellSpacing,_NavTableCellPadding,_NavTableLayerWidth,_NavTableLayerHeight,_NavTableLayerTop,_NavTableLayerLeft;
    ...
    public void clearSelectedItem() {
        ...
        HtmlLink hl = null;
        HtmlImage hi = null;
        var en = hc.getComponents();
        while (en.hasMoreElements()) {
            HtmlComponent hcp = (HtmlComponent) en.nextElement();
        ...
        try {
            for (int i = 0; i < _popups.size(); i++) {
                ...
                if (hpPop.getName().equals(name)) {
                    var en = hpPop.getComponents();
                    nmReturn = (NavigationMenu) en.nextElement();
                    break;
                }
            }
        } catch (Exception e) {
...
/*	HtmlTableCellProperties navLinkTableCellProp = new HtmlTableCellProperties();
	...
	navLinkTableCellProp.setStyle(_cellStyle);*/
        ...
        if (submenu != null && !submenu.trim().equals("")) {
            ...
            var en = getPage().getComponents();
            while (en.hasMoreElements()) {
                HtmlComponent hc = (HtmlComponent) en.nextElement();
...
/*		  String sMouseOver="this.className='"+_cellHoverStyle.getStyleName()+"';"+anchor.getFullName()+".className='"+_linkcellhoverstyle.getStyleName()+"';";
    ...
    public void removePopups() {
        for (int i = 0; i < _popups.size(); i++) {
            HtmlPopup hpPop = (HtmlPopup) _popups.elementAt(i);
            var en = hpPop.getComponents();
            NavigationMenu nm = (NavigationMenu) en.nextElement();
            nm.removePopups();
...
/*  String sItemKey=_groupname+".Group"+groupIndex+".Item"+itemIndex;
  ...
  Enumeration enum=hc.getComponents();
  while(enum.hasMoreElements()) {
	HtmlComponent hcp=(HtmlComponent)enum.nextElement();
	if (hcp instanceof HtmlLink) {
    ...
    public void setSelectedItem(int groupIndex, int itemIndex) {
        ...
        while (en.hasMoreElements()) {
            ...
            if (hcp instanceof HtmlLink) {
                hl = (HtmlLink) hcp;
                var enumLink = hl.getComponents();
                outer:
                while (enumLink.hasMoreElements()) {
                    HtmlComponent hcpLink = (HtmlComponent) enumLink.nextElement();
                    if (hcpLink instanceof HtmlTable) {
                        HtmlContainer hcCont = (HtmlContainer) ((HtmlTable) hcpLink).getComponents().nextElement();
                        var enumCont = hcCont.getComponents();
                        while (enumCont.hasMoreElements()) {
                            HtmlComponent hcpCont = (HtmlComponent) enumCont.nextElement();
                            if (hcpCont instanceof HtmlLink) {
                                hl = (HtmlLink) hcpCont;
  ...
  while(enum.hasMoreElements()) {
	...
	if (hcp instanceof HtmlLink) {
	  ...
	  while(linkEnum.hasMoreElements()) {
		HtmlComponent hcpLink=(HtmlComponent)enum.nextElement();
