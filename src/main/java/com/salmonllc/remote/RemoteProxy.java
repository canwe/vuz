 ...
 */
/**
 * This class provides a way to reference an object in the session on the server. Access is achieved via the RemoteReflector servlet.
...
public class RemoteProxy implements Serializable {
    ...
    private URLConnection openConnection(String method,Class[] caParms,Object[] oaParms,boolean bInstantiate) throws Exception {
        ...
        if (_proxyUserID != null) {
            ...
            String auth = "Basic " + BASE64Encoder.encode(authString);
            conn.setRequestProperty("Proxy-Authorization", auth);
        }

        if (_user!=null) {
            String sEncodedUserPassword=null;
            sEncodedUserPassword="Basic "+BASE64Encoder.encode((_user+":"+_pass));
