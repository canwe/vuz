...
public class DataStoreBuffer implements Serializable {
	...
	public String export() throws DataStoreException {
	...
	public void export(PrintWriter p) throws DataStoreException {
		String TAB = "      ";
	 ...
	 * @param column The column name in the data store buffer.
	 */
	public Timestamp getDateTime(String column) throws DataStoreException {
		return getDateTime(getRow(), column);
	}
	/**
	...
	public Properties getProperties() {
		...
		for (int i = 0; i < _desc.getColumnCount(); i++) {
			...
			if (f instanceof SimpleDateFormat)
				p.put(name + "format", getStringProperty(((SimpleDateFormat) f).toPattern()));
			else if (f instanceof DecimalFormat)
				p.put(name + "format", getStringProperty(((DecimalFormat) f).toPattern()));
			else
	 ...
	 * @param column The column number in the data store buffer.
	 */
	public Time getTime(int column) throws DataStoreException {
	 ...
	 * @param column The column number in the data store buffer.
	 */
	public Time getTime(int row, int column) throws DataStoreException {
	 ...
	 * @param column The column name in the data store buffer.
	 */
	public Time getTime(int row, String column) throws DataStoreException {
		Object o = getObject(row, column, DATATYPE_TIME);
		if (o == null)
			return null;
		else
	 ...
	 * @param column The column name in the data store buffer.
	 */
	public Time getTime(String column) throws DataStoreException {
		return getTime(getRow(), column);
	}
	 ...
	 */
	public void importRow(Row row) throws DataStoreException {
		// Insert a row
		// Then set row data
		int i = insertRow();
	...
	*/
	 ...
	 * @param value The data to place in the column.
	 */
	public void setDateTime(int row, int column, Timestamp value) throws DataStoreException {
	 ...
	 * @param value The data to place in the column.
	 */
	public void setDateTime(int row, String column, Timestamp value) throws DataStoreException {
		setObject(row, column, value, DATATYPE_DATETIME);
	}
	/**
	 * This method sets a value in the data store's internal buffer at the current row.
	 ...
	 * @param value The data to place in the column.
	 */
	public void setDateTime(int column, Timestamp value) throws DataStoreException {
	 ...
	 * @param value The data to place in the column
	 */
	public void setDateTime(String column, Timestamp value) throws DataStoreException {
		setObject(getRow(), column, value, DATATYPE_DATETIME);
	}
	/**
	...
	public void setFormat(int col, String format) throws DataStoreException {
		...
		if (d.getType() == DATATYPE_INT || d.getType() == DATATYPE_LONG || d.getType() == DATATYPE_DOUBLE || d.getType() == DATATYPE_SHORT || d.getType() == DATATYPE_FLOAT)
			d.setFormat(new DecimalFormat(format));
		else if (d.getType() == DATATYPE_DATE || d.getType() == DATATYPE_TIME || d.getType() == DATATYPE_DATETIME) {
			SimpleDateFormat df = new SalmonDateFormat(format);
			df.setLenient(false);
			d.setFormat(df);
		} else
	...
	public void setFormattedString(int row, int column, String value) throws DataStoreException {
		...
		try {
			if (value == null)
				...
				if (f instanceof DecimalFormat) {
					Number parsed = ((DecimalFormat) f).parse(value);
					...
					if (colType == DATATYPE_DATE)
						...
						setTime(row, column, new Time(parsed));
					else if (colType == DATATYPE_DATETIME)
						setDateTime(row, column, new Timestamp(parsed));
				...
				try {
					...
					if (type == DATATYPE_INT)
						...
						try {
							...
							MessageLog.writeDebugMessage("setObject() - received date expecting timestamp performed conversion", this);
						}
					} else if (type == DATATYPE_TIME)
						...
						try {
							...
							MessageLog.writeDebugMessage("setObject() - received timestamp expecting date performed conversion", this);
						}
					}
					//					data = java.sql.Date.valueOf((String) data);
					else
					...
					data = null;
					MessageLog.writeErrorMessage("setObject()", e, this);
				}
			}
		}
	 ...
	 * @param value The data to place in the column.
	 */
	public void setTime(int row, int column, Time value) throws DataStoreException {
	 ...
	 * @param value The data to place in the column.
	 */
	public void setTime(int row, String column, Time value) throws DataStoreException {
		setObject(row, column, value, DATATYPE_TIME);
	}
	/**
	 * This method sets a value in the data store's internal buffer at the current row.
	 ...
	 * @param value The data to place in the column.
	 */
	public void setTime(int column, Time value) throws DataStoreException {
	 ...
	 * @param value The data to place in the column
	 */
	public void setTime(String column, Time value) throws DataStoreException {
		setObject(getRow(), column, value, DATATYPE_TIME);
	}
	/**
	...
	*/
	...
	private boolean compare(Object com1, Object com2) {
		if ((com1 == null) && (com2 == null)) {
			...
			if (com1.equals(com2))
				...
				return true;
		} else if (com1 instanceof Time) {
			if (! (com2 instanceof Time))
			  return false;

			if (com1.equals(com2))
				return false;
			else if (((Time) com1).before((Time) com2))
				return false;
			else
				return true;
		} else
	...
	public static Object convertValue(String val, int dataType) throws ParseException {
		...
		if (dataType == DataStoreBuffer.DATATYPE_TIME)
			return new Time(new SimpleDateFormat("hh:mm").parse(val).getTime());
...
//fc 06/11/04: Copies the master data values from one row to another. Used for one to many relationship models.
    void copyMasterDataValuesToRow(TablesRelationshipDescription dsTables, DataStoreRow fromrow, DataStoreRow torow) throws DataStoreException {
...
//fc 06/11/04: Copies the master key values from one row to another. Used for one to many relationship models.
    void copyMasterKeyValuesToRow(TablesRelationshipDescription dsTables, DataStoreRow fromrow, DataStoreRow torow, int iTableRow) throws DataStoreException {
        String sMasterColumns=dsTables.getString(iTableRow, TablesRelationshipDescription.MASTERKEYCOLUMNS);
...
//fc 06/11/04: Copies the master data values from one row to another. Used for one to many relationship models.
    void copyMasterDataValuesToRow(TablesRelationshipDescription dsTables, DataStoreRow fromrow, DataStoreRow torow, int iTableRow) throws DataStoreException {
        String sTable=dsTables.getString(iTableRow, TablesRelationshipDescription.TABLE)+".";
