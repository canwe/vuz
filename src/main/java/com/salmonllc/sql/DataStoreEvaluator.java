 ...
 * The class needs to be passed a String containing the expression that will be used. This expression will be parsed and evaluated using the evaluate row method.The rules for expressions are as follows:<BR><BR>
 * Column names: Columns in the datastore can be referenced in the same way that they are referenced in the get and set methods in the datastore. Generally this is either by using tablename.columnname or a single intenral name string. Which you can use depends on how the column was added to the datastore.<BR><BR>
 * Comparisons: You can use ==, >=, <=, !=, > or < comparisons in your expressions. Use these to compare the values of Strings, Dates, Numbers or Booleans. (Note: The values of each part of the comparison will be used as opposed to the default java method which will test object references).<BR><BR>
 * Operators: You can use +, -, /, * and ^ (exponent) to perform operations on numeric values. The + operator may also be used to concatinate String values.<BR>
 *    In addition the if operator can be used to return a value based on a comparison. The syntax for if is : if(comparison,truevalue,falsevalue)<BR><BR>
 * Literals: String Literals are surrounded by double quotes, Date, DateTime or Time Literals are also surrounded by quotes and in JDBC date escape form YYYY-MM-DD for dates, HH:MM:SS for times and a combination of the two for DateTimes<BR><BR>
 * Nulls: The constant "null" can be used to check for columns values that are filled in. The equals(==) and not equals(!=) comparisons will return valid results when used with null. Every other comparison returns false when used with a null value on either side of the expression.<BR><BR>
 ...
 * Examples:<BR>
 *		  <pre>
 *        "table1.column1 == 'xxxx'"<BR>
 * 		  "table1.column1.substring(2,3) + 'xxx'"<BR>
 *		  </pre>
 * String Methods:<BR>
 * String substring(int start, int end): Gets a section of the String starting at start and ending at the character before end.<BR>
 * Integer indexOf(String subString) : Finds the first position of subString in the string or -1 if no occurance is found.<BR>
 * Integer lastIndexOf(String substring) : Finds the last position of subString in the string or -1 if no occurance is found.<BR>
 * Integer length() : Finds the length of the string in characters.<BR>
 * String toUpperCase(): Returns an all upper case version of the string.<BR>
 * String urlEncode(): Replaces ampersand, questionmarks and spaces in a String with URL Encoded Characters<BR>
 * String toLowerCase(): Returns an all lower case version of the string.<BR>
 * String trim(): Returns all leading and trailing spaces from the string.<BR>
 * String charAt(int position): Returns the character at position.<BR>
 * String startsWith(String substring): Returns true if the string starts with the specified substring.<BR>
 * String endsWith(String substring): Returns true if the string ends with the specified substring.<BR>
 * String escape(): Returns a string with quotes and spaces escaped so they work in javascript code.<BR>
 * String tempValue(): Returns a string with the temporary value of the column<BR>
 * Numeric Methods:<BR><BR>
 * Double sqrt(): Returns the square root of the number.<BR>
 * Double round(int places): Returns the number rounded to "places" number of decimal places.<BR>
 * String format(String pattern): Format the value as a string using the specified pattern.<BR><BR>
 * Date Methods:<BR><BR>
 * Integer getMonth(): Returns the month of the given date (January = 0, Dec = 11).<BR>
 * Integer getDate(): Returns the day of the month of the given date.<BR>
 * Integer getDay(): Returns the day of the week of a given date (Sunday = 0, Saturday = 6).<BR>
 * Integer getYear(): Returns the year of the given date.<BR>
 * Integer getHours(): Returns the hours of the given time.<BR>
 * Integer getMinutes(): Returns the Minutes of the given time.<BR>
 * Integer getSeconds(): Returns the Seconds of the given time.<BR>
 * Date toDate(): Returns the Date portion of a DateTime.<BR>
 * Time toTime(): Returns the Time portion of a DateTime.<BR>
 * Date dateAdd(int days): Returns the specified date incremented by "days" number of days.<BR><BR>
 * String format(String pattern): Format the value as a string using the specified pattern.<BR><BR>
 * There are also five constants you can use<BR>
 * null: The value null<BR>
 * $DATE$:todays date as a Date value<BR>
 * $TIME$: todays date and time as Datetime value<BR>
 * $ROWNO$: the number of the row that the evaluator is evaluating<BR>
 * $ROWCOUNT$: the total number of the rows in the datastore<BR>
 * $ROWSTATUS$: the status of the row. See DatastoreBuffer.STATUS constants<br> 
 * $QUOT$: include a single quote in a string<BR>
 */
    ...
    public Object evaluateRow(int rowNo) throws DataStoreException {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (token.equals("null"))
                ...
                _st.push(Timestamp.valueOf(token.substring(1, token.length() - 1)));
...
//new method
     ...
     * @throws DataStoreException
     */
    private Date functionDateAdd(Date d) throws DataStoreException {
     ...
     * @throws DataStoreException
     */
    private Integer functionGetDate(Date d) throws DataStoreException {
     ...
     * @throws DataStoreException
     */
    private Integer functionGetDay(Date d) throws DataStoreException {
        if (d instanceof java.sql.Time)
...
//	return new Integer(d.getDay());
     ...
     * @throws DataStoreException
     */
    private Integer functionGetHours(Date d) throws DataStoreException {
     ...
     * @throws DataStoreException
     */
    private Integer functionGetMinutes(Date d) throws DataStoreException {
        if (d instanceof java.sql.Date)
     ...
     * @throws DataStoreException
     */
    private Integer functionGetMonth(Date d) throws DataStoreException {
        if (d instanceof java.sql.Time)
     ...
     * @throws DataStoreException
     */
    private Integer functionGetSeconds(Date d) throws DataStoreException {
        if (d instanceof java.sql.Date)
     ...
     * @throws DataStoreException
     */
    private Integer functionGetYear(Date d) throws DataStoreException {
        if (d instanceof java.sql.Time)
     ...
     * @throws DataStoreException
     */
    private java.sql.Date functionToDate(Date d) throws DataStoreException {
    ...
    private Timestamp functionCurrentTime() {
     ...
     * @throws DataStoreException
     */
    private java.sql.Time functionToTime(Date d) throws DataStoreException {
    ...
    public static boolean isDateTime(String token) {
        if (token.length() > 0) {
            if ((token.charAt(0) == '"' || token.charAt(0) == '\'') && token.charAt(token.length() - 1) == token.charAt(0)) {
                try {
                    Timestamp.valueOf(token.substring(1, token.length() - 1)); // unused
                    return true;
                } catch (Exception e) {
                    return false;
                }
