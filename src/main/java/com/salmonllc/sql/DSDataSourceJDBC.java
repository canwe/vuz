...
abstract class DSDataSourceJDBC implements DataSourceIn, DataSourceOut, Serializable {
	...
	private PreparedStatement prepareDelete(DataStore ds, DataStoreRow row, String tableIn, DBConnection conn, StringBuffer sqlb) throws SQLException, DataStoreException {
	 ...
	 *            int
	 */
	static void prepareForType(DataStore ds, PreparedStatement pst, Object data, int type, int pos) throws SQLException {
		//fc 06/24/04
		...
		switch (type) {
			...
			case DataStore.DATATYPE_DATETIME :
				if (data == null)
					pst.setNull(pos, Types.TIMESTAMP);
				else
					pst.setTimestamp(pos, (Timestamp) data);
				break;
			case DataStore.DATATYPE_DATE :
				if (data == null)
					pst.setNull(pos, Types.DATE);
				else
					pst.setDate(pos, (Date) data);
				break;
			case DataStore.DATATYPE_TIME :
				if (data == null)
					pst.setNull(pos, Types.TIME);
				else
					pst.setTime(pos, (Time) data);
				break;
			case DataStore.DATATYPE_DOUBLE :
				if (data == null)
					pst.setNull(pos, Types.DOUBLE);
				else
	...
	private PreparedStatement prepareInsert(DataStore ds, DataStoreRow row, String tableIn, DBConnection conn, StringBuffer sqlb, PreparedStatement pin) throws SQLException, DataStoreException {
		DSColumnDescriptor col;
		Object data;
		int type;
		String value = "";
	...
	private PreparedStatement prepareUpdate(DataStore ds, DataStoreRow row, String tableIn, DBConnection conn, StringBuffer sqlb) throws SQLException, DataStoreException {
