...
/**
 * Use this class to represent an SQL DATE or TIMESTAMP value as a series of drop-down
...
public class HtmlDateComponent extends HtmlFormComponent {
	...
	public HtmlDateComponent(String name, HtmlPage p, int nbrIntervals, int iFromTime, int iToTime, int iFlag) {
		...
		if (nbrIntervals <= 0) {
			...
			if (dRemainder != 0)
				MessageLog.writeDebugMessage("Interval in HtmlDateComponent is not " + "fraction of the day. interval = " + nbrIntervals, this);
		...
		if ((month == null && day == null && year == null && hour == null)) {
			...
			switch (_dsDataType) {
				case DataStore.DATATYPE_DATE :
					_dsTemp.setDate(0, new Date(gc.getTime().getTime()));
					break;
				case DataStore.DATATYPE_DATETIME :
					_dsTemp.setDateTime(0, new Timestamp(gc.getTime().getTime()));
					break;
			}
