...
public abstract class HtmlPageBase implements Serializable
{
	...
	*/
	...
	public void setApplicationAlias(String applicationAlias) {
		...
		Hashtable h = (o == null ? new Hashtable() : (Hashtable) o);
		...
		if (pr == null) {
			...
			if (isRequestFromPortlet()) {
				...
				if (info.getRenderRequest() != null)
					en =	getPortletInfo().getRenderRequest().getLocales();
				else if (info.getActionRequest() != null)
					en =	getPortletInfo().getActionRequest().getLocales();
				lang=SalmonPortlet.constructLanguageString(en);
			}	
			else
