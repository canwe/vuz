...
public class HtmlContainer extends HtmlComponent {
	...
	public HtmlContainer(String name, HtmlPage p) {
		super(name, p);
	}
	/**
	...
	public HtmlComponent getComponent(String name) {
		...
		// we will search all items in this enumeration
		// recursivly calling the containers getComponent method
		boolean breakout = en.hasMoreElements();
		while (breakout && en.hasMoreElements()) {
			ret = null;
			
			comp = (HtmlComponent) en.nextElement();
