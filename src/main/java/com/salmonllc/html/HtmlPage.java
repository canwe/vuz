...
/**
 * This type is the superclass for every page in the HTML Framework. The developer creating the page should override the initialize method.
 * In this method you should create and place any HTMLComponents that will reside in the page plus any other required initialization. The rest of the custom code in the page
 * will be handled via listeners.
 */
public abstract class HtmlPage extends HtmlPageBase implements Serializable {
	 ...
	 * @exception DataStoreException The exception description.
	 */
	...
	*/
	...
	public void writeNamedScripts(Writer pw) throws IOException {
		if (_scriptTable != null) {
			var en =_scriptTable.elements();
			while (en.hasMoreElements()) {
				String script=(String)en.nextElement();
				pw.write(script);
				pw.write("\r\n\r\n");	
			}
			_scriptTable.clear();		
	...
	public void writeNamedHtml(Writer pw) throws IOException {
		if (_htmlTable != null) {
			var en =_htmlTable.elements();
			while (en.hasMoreElements()) {
				String html=(String)en.nextElement();
				pw.write(html);
			}
			_htmlTable.clear();		
		}	
