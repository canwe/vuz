 ...
 * Time: 12:45:35
 */
public class AddressController extends BaseVocabulariesController
		implements SubmitListener, PageListener {
	//Visual Components
	public com.salmonllc.html.HtmlDropDownList _settlementSelect;
	public com.salmonllc.html.HtmlImage _sourceForgeLogoImage;
	public com.salmonllc.html.HtmlLink _addresses;
	public com.salmonllc.html.HtmlText _addressCAP;
	public com.salmonllc.html.HtmlText _addressesCAP;
	...
	public com.salmonllc.html.HtmlText _settlements;
	public com.salmonllc.html.HtmlTextEdit _address;
	public com.salmonllc.html.HtmlTextEdit _searchEdit;
	public com.salmonllc.html.HtmlTextEdit _settlement;
...
//DataSources
	public com.itm.vuz.vocabularies.models.HibernateAddressModel _hibernateAddressDatasource;
	public com.itm.vuz.vocabularies.models.HibernateSettlementModel _hibernateSettlementModel;
...
//DataSource Column Constants
	...
	public boolean submitPerformed(SubmitEvent event) {
        ...
        if(event.getSource() == _d.getDeleteButton()){
            ...
            // address
            Address address = (Address)_hibernateAddressDatasource.getDataRow(rowNo).getBean();
            MessageLog.writeDebugMessage("trying to delete address with name = [" +
                    address.getAddress() + "]", this);

              //          address.getSettlement().getArea().getRegion().
            if(address.getStudents()!=null && address.getStudents().size()>0){
                ...
                try{
                    _d.getValidator().addErrorMessage("Невозможно удалить адрес " + address.getAddress() + ". Есть информация о студентах, проживающих по этому адресу, сперва удалите её.");
                    _d.tryCancel();
                }catch(Exception e){
	...
	private void initializeSettlementComboBox() throws Exception {
		_settlementSelect.resetOptions();
		_hibernateSettlementModel.reset();
        _hibernateSettlementModel.retrieve();
        _settlementSelect.addOption(null,"");
		for (int i = 0; i < _hibernateSettlementModel.getRowCount(); i++) {
			Settlement settlement =
					(Settlement) _hibernateSettlementModel.getDataRow(i).getBean();
			if (settlement != null) {
				_settlementSelect.addOption(settlement.getSettlementId().toString(),
						settlement.getName());
			}
